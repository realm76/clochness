...
type NodeQuery struct {
	...
	withTimeEntries *NodeTimeEntryQuery
...
func (nq *NodeQuery) QueryTimeEntries() *NodeTimeEntryQuery {
	query := (&NodeTimeEntryClient{config: nq.config}).Query()
...
func (nq *NodeQuery) Clone() *NodeQuery {
	...
	return &NodeQuery{
		...
		withTimeEntries: nq.withTimeEntries.Clone(),
...
func (nq *NodeQuery) WithTimeEntries(opts ...func(*NodeTimeEntryQuery)) *NodeQuery {
	query := (&NodeTimeEntryClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withTimeEntries = query
	return nq
}
...
//
//	client.Node.Query().
//		GroupBy(node.FieldParentHandle).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (nq *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	...
	if query := nq.withTimeEntries; query != nil {
		if err := nq.loadTimeEntries(ctx, query, nodes,
			func(n *Node) { n.Edges.TimeEntries = []*NodeTimeEntry{} },
			func(n *Node, e *NodeTimeEntry) { n.Edges.TimeEntries = append(n.Edges.TimeEntries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NodeQuery) loadTimeEntries(ctx context.Context, query *NodeTimeEntryQuery, nodes []*Node, init func(*Node), assign func(*Node, *NodeTimeEntry)) error {
	...
	query.withFKs = true
	query.Where(predicate.NodeTimeEntry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(node.TimeEntriesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.node_time_entries
		if fk == nil {
			return fmt.Errorf(`foreign-key "node_time_entries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "node_time_entries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
